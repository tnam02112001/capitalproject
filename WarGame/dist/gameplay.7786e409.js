...
var Deck = /*#__PURE__*/function () {
  function Deck() {
    var cards = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : newDeck();
...
var Card = /*#__PURE__*/function () {
  ...
  _createClass(Card, [{
    ...
    value: function divGenFlipped() {
      ...
      cardDiv.dataset.value = "".concat(this.val, " ").concat(this.suit);
      return cardDiv;
    }
  }, {
    key: "divGenUnflipped",
    value: function divGenUnflipped() {
      var cardDiv = document.createElement("div");
      cardDiv.setAttribute('id', this.val + this.suit);
      cardDiv.innerText = "?";
...
},{"./lib/axios":"node_modules/axios/lib/axios.js"}],"gameplay.js":[function(require,module,exports) {
"use strict";
...
var p1DeckElement = document.querySelector(".p1-deck");
var p2DeckElement = document.querySelector(".p2-deck");
var statusElement = document.querySelector(".status");
var p1ScoreElement = document.querySelector(".p1Score");
var p2ScoreElement = document.querySelector(".p2Score");
var p1Deck, p2Deck, p1Cards, p2Cards, p1Score, p2Score, game, btStart, btStop;
var inRound = false;
var cardPulled = false;
var inWar = false;
var inGame = false; // Database setup

setUpScore(); // Generate functional buttons
...
function setUpScore() {
  ...
  _axios.default.get("http://localhost:5000/", {
    params: {
      "name": "Player 1"
    }
  }).then(function (resp) {
    console.log(resp.data.score);
    if (resp.data.score == -1) p1Score = initializePlayer("Player 1");else console.log(resp.data.score);
  ...
  _axios.default.get("http://localhost:5000/", {
    ...
    if (resp.data.score == -1) p2Score = initializePlayer("Player 2");else p2Score = resp.data.score;
  });
...
function initializePlayer(playerName) {
  ...
  _axios.default.post("http://localhost:5000/", player).then(function (response) {
...
function genrateButtons() {
  ...
  document.querySelector(".title").insertAdjacentElement('afterend', btStop); // Generate the startGame button
...
function pullCards(n) {
  for (var i = 0; i < n; i++) {
    var p1Card = p1Deck.popCard();
    var p2Card = p2Deck.popCard();
    p1Cards.push(p1Card);
    p2Cards.push(p2Card);
...
function flipCards() {
  inRound = true;
  var p1Card = p1Cards[p1Cards.length - 1];
  var p2Card = p2Cards[p1Cards.length - 1];
  var p1CardUnflippedElement = document.getElementById(p1Card.val + p1Card.suit);
  var p2CardUnflippedElement = document.getElementById(p2Card.val + p2Card.suit);
...
function startGame() {
  var deck = new _deck.default();
  deck.shuffle();
  var mid = Math.ceil(deck.cards.length / 2);
  p1Deck = new _deck.default(deck.cards.slice(0, mid));
  p2Deck = new _deck.default(deck.cards.slice(mid, deck.cards.length));
  cleanBeforeRound();
}
...
function checkRound(p1Card, p2Card) {
  if (CARD_DICT[p1Card.val] > CARD_DICT[p2Card.val]) {
    ...
    p1Cards.forEach(function (p1Card) {
      return p1Deck.addCard(p1Card);
    });
    p2Cards.forEach(function (p2Card) {
      return p1Deck.addCard(p2Card);
    });
    p1Score++;
    ...
    p1Cards.forEach(function (p1Card) {
      return p2Deck.addCard(p1Card);
    });
    p2Cards.forEach(function (p2Card) {
      return p2Deck.addCard(p2Card);
    });
    inWar = false;
    p2Score++;
...
function updateScore(playerName, currScore) {
  ...
  _axios.default.put("http://localhost:5000/", player).then(function (response) {
    console.log(response);
  }).catch(function (error) {
    console.log(error);
  });
}
},{"./deck.js":"deck.js","./node_modules/axios":"node_modules/axios/index.js"}],"node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["node_modules/parcel-bundler/src/builtins/hmr-runtime.js","gameplay.js"], null)
//# sourceMappingURL=/gameplay.7786e409.js.map